...
- MVP **wraps existing runtimes**; it does NOT bundle Node/Python/Telethon/Selenium/LibreWolf. Target machine already has this repo plus those dependencies. A self-contained installer that ships the runtimes is a later phase.
...
2. Setup wizard — API creds form (link my.telegram.org) → launch login (console) → pick `sourceChatId` → capture `adminUserIds` → bot token + `reportChatId` → validate → send test → enable toggle.
...
Outbound `sendMessage` only; never `getUpdates`/`setWebhook`/`deleteWebhook`; secrets never printed or committed; invite URLs never logged.
