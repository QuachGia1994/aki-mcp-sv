...
| Phạm vi filesystem — chọn linh hoạt mỗi session | **Được ở mức khởi chạy process, KHÔNG ở mức mỗi prompt.** MCP filesystem server chốt root path lúc start (đây là ranh giới bảo mật cố ý của MCP — nếu client tự chọn path mỗi lần gọi tool thì whitelist mất tác dụng, AI có thể xin bất kỳ path nào). Linh hoạt hoá bằng biến môi trường: `MCP_DATA_DIR=/path/khac npm start` — đổi scope mỗi lần khởi chạy, vẫn cố định trong suốt phiên đang chạy. **Đổi 2026-08-07**: default chuyển từ `./data` sang `/Volumes/DEV` (toàn volume code thật) vì `./data` gần như không dùng trong công việc thực tế — xem `README.md`. `list_allowed_directories`/`directory_tree` (tool có sẵn của filesystem MCP server) là cơ chế "index" tự động: Claude tự gọi để biết phạm vi + tự khám phá bên trong, không cần index riêng. Cố ý loại `$HOME/.claude` khỏi mọi scope (chứa `auth-cache.json`/`.claude.json`/`history.jsonl` — token, lịch sử chat) | default `/Volumes/DEV` chốt ở `package.json` (`start` script); `mcp-hub.config.json` chỉ đọc `${MCP_DATA_DIR}` trần, không tự có fallback |
