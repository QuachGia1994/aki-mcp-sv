...
Two smallest-honest slices of this plan are live, both keying off the single `origin` value so no OAuth/gatekeeper/bridge code changed — exactly the "ingress is a swappable edge" property this doc argued for.

**`PUBLIC_ORIGIN` (manual).** `scripts/start.js` reads `PUBLIC_ORIGIN`, and when set, skips Tailscale entirely and uses that origin as-is. A user who already runs a Cloudflare Tunnel (or any stable TLS edge) points it at `127.0.0.1:9999` and exports `PUBLIC_ORIGIN=https://their-host`; setup is theirs, not automated by this repo. Approach contributed via PR #4 (`@Ran-Xing`); the FRP/local-TLS path from that PR was dropped since a Cloudflare edge terminates TLS itself.

**`cloudflared` named-tunnel launcher (opt-in).** `npm start -- --tunnel <cred.json> --origin https://your-host` reads `TunnelID` from the cloudflared credentials JSON and spawns `cloudflared tunnel run --cred-file <path> --url http://127.0.0.1:9999 <TunnelID>`. The child is managed like the mcp-hub child — killed on shutdown and via the process `exit` safety net; a missing `cloudflared` binary (ENOENT) prints an install hint, and an unexpected cloudflared exit tears the whole stack down. Ingress precedence is `--tunnel` > `PUBLIC_ORIGIN` > Tailscale Funnel (the default). In these non-Funnel modes the panel's section 0 drops the Tailscale install/funnel checks, the Recheck button, and the re-sync command, and shows a small "Custom ingress active" block naming the mode and the serving origin instead — fixing the prior false-failed-Tailscale report for a user who intentionally ran their own tunnel.

Deliberately the most minimal launcher that works, not the `MCP_INGRESS` env-var sketch under "Minimal design" below: only the JSON-credentials mode is supported — no yml config path and no token path. A yml is unnecessary because the hostname is supplied via `--origin` and the forward port (9999) is fixed and passed by start.js as `--url`; token mode was dropped to keep a single code path. `--origin` is required because a credentials JSON carries no hostname field (only `AccountTag`, `TunnelSecret`, `TunnelID`, `Endpoint`), so `--tunnel` without `--origin` exits with an error. `cloudflared` is shelled out as one external binary, consistent with the repo's "shell out to a real binary, never reimplement" rule.

What is still design only: whether a Cloudflare Tunnel actually removes the intermittent per-request drops. This launcher makes the tunnel easy to stand up; it does not prove the reliability win — the drop-rate test further down has not been run, so that claim stays unverified. A managed fixed-subdomain offering (own domain, per-user subdomain, branded favicon at the apex) is a separate future project, not part of this repo's zero-config default.
...
Both escape hatches: shipped (under `[Unreleased]`, above) — the manual `PUBLIC_ORIGIN` override and the opt-in `cloudflared` named-tunnel launcher, each a low-cost edge swap that leaves OAuth/gatekeeper/bridge untouched. The launcher ships in its most minimal form: JSON-credentials only, `--origin` supplying the hostname, one shelled-out `cloudflared` binary. Funnel remains the default. What is NOT settled: whether a Cloudflare Tunnel measurably fixes the per-request drops — the drop-rate test above is still design only, so the reliability win stays unproven, not claimed. Whether the setup cost is worth it for the users who hit the drops most is decided by that test, not by shipping the launcher.
