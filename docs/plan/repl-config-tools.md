...
| `get_config` | Tool mới, read-only, trả JSON `{platform, shell, pythonVersion, nodeVersion, repoRoot}` — `repoRoot` cứng luôn là `/Volumes/DEV/Nodejs/aki-mcp-sv` (hằng, không đọc từ file) | Rẻ, rủi ro gần 0. Gộp luôn `repoRoot` vì đây là kênh đáng tin duy nhất để 1 phiên Claude khác tự định vị repo — xem mục "`instructions` field — vì sao không dùng" bên dưới, không dùng README hay `instructions` field cho việc này |
...
- `README.md` — setup, vị trí repo (`/Volumes/DEV/Nodejs/aki-mcp-sv`)
