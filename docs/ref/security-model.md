...
Postman pool auto-join is another opt-in local automation plane, not MCP authentication. When `~/.aki/mcpsv/postman-pool.json` is enabled, a Telethon user-session listener receives only the owner's source-group messages and `scripts/postman-pool.js` still requires both the exact source chat ID and an allowlisted sender user ID before it accepts an invite. The source group never needs the report bot. The bot is outbound-only (`sendMessage`); Aki never calls `getUpdates`, `setWebhook`, or `deleteWebhook`, so a pre-existing ROBOT SLTP webhook can keep exclusive inbound ownership of the same bot token. Telegram API credentials/session and the report bot token stay user-local; invite URLs are never logged and reach the transient Chrome worker through stdin. The worker launches only a dedicated non-default persistent Chrome user-data directory and binds its CDP endpoint to loopback. It does not parse/decrypt cookies, access tokens, or saved passwords; only temporary Postman identity-history copies are read under the configured scratch root. Human Verify remains a manual checkpoint; CDP is limited to normal Postman navigation and its invite control, not challenge interaction. See `docs/ref/postman-pool-autojoin.md`.
...
- `docs/ref/postman-pool-autojoin.md` — Telegram allowlist, invite handling, dedicated Chrome/CDP browser boundary, manual Human Verify checkpoint, and join/report behavior
