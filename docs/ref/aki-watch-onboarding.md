...
Aki Watch is the Windows-only Postman pool auto-join: it listens to one Telegram group through your own Telegram user session, and when an allowlisted admin posts a Postman team invite it opens that invite in each local LibreWolf Postman profile, accepts it, and reports the joined accounts back through a Telegram bot DM. This guide is the human walkthrough; the architecture and runtime flow live in `docs/ref/postman-pool-autojoin.md` (how the pieces fit and why).
...
| Sign each of the ~13 LibreWolf profiles into Postman | Automating a Postman login (credentials/SSO/2FA) is a security and terms risk; the automation only accepts invites in profiles already signed in. |
...
- LibreWolf installed (default `C:\Program Files\LibreWolf\librewolf.exe`), with your ~13 Postman profiles already signed in.
- A roomy non-system drive for `scratchRoot` (profile clones are created and deleted per run).
...
Ensure each LibreWolf profile you want in the pool is already logged into Postman in the browser. The automation clicks only `Accept Invite` / `Join Team`; if a profile is signed out it is reported as failed, never fed credentials.
...
`--check` is non-interactive: it prints Node/Python/Telethon/Selenium/LibreWolf readiness and the current config state (which required fields are still missing), without printing any secret value. Run it any time to see whether the watcher can be enabled.

```powershell
node scripts/postman-pool-setup.js
```

The interactive wizard walks Step 1→6, reusing the proven scripts: it runs the Telethon login and dialog list, helps you pick `sourceChatId`, observes the group to capture the admin `adminUserIds`, takes the bot token and `reportChatId`, validates the config, offers a single outbound `sendMessage` test, and only then offers to set `enabled=true`. Secrets are written to `~/.aki/mcpsv/postman-pool.json` (outside git) and never echoed. After enabling, restart Aki so the watcher process reads the new config.
