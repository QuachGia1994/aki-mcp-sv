# Kimi K3 Web -> Aki bootstrap

Canonical browser bootstrap for Kimi K3 through `https://aki-bridge.oakgatekeeper.uk`.

Before running, set `BRIDGE_TOKEN` to the Kimi-specific bearer and set `PROJECT` to the absolute Windows project path. Do not reuse another provider's bearer and do not commit runtime credentials.
...
BRIDGE_TOKEN = "PASTE_KIMI_BEARER_HERE"
...
After bootstrap, use the `aki()` helper for all real Windows project access. Use `local__find_path` for discovery, `local__search_content` for source search, filesystem tools for reads/edits/writes, and `local__run_cmd` with `cwd=PROJECT` for Git/tests/builds. Never fall back to Kimi's Linux workspace. Retry timed-out task creation only with the same Idempotency-Key.
