...
**Contents:** [Why this exists](#why-this-exists) · [When to use & Core Use-Cases](#when-to-use--core-use-cases) · [Install](#install) · [Run](#run) · [Connecting from Claude web](#connecting-from-claude-web) · [Connecting from ChatGPT](#connecting-from-chatgpt) · [Connecting from Grok, Gemini Spark, and Antigravity 2.0](#connecting-from-grok-gemini-spark-and-antigravity-20) · [Connecting from Postman](#connecting-from-postman) · [xKiro Free worker](#xkiro-free-worker) · [Aki Free-first Orchestrator](#aki-free-first-orchestrator) · [Kimi Web K3 via Cloudflare D1](#kimi-web-k3-via-cloudflare-d1) · [Qwen Coder Web via Worker + D1](#qwen-coder-web-via-worker--d1) · [Autonomous Cloud Automation](#autonomous-cloud-automation-grok--local-mcp) · [Requirements](#requirements) · [Architecture](#architecture) · [Directory layout](#directory-layout) · [Configuration](#configuration) · [Exposing to the internet](#exposing-to-the-internet) · [Finding files](#finding-files) · [Security](#security)
...
On a fresh install with no saved `folders`, the main allowed root is only the process working directory (`process.cwd()`), or the explicit comma-separated paths in `MCP_DATA_DIR` when that variable is set. Aki additionally grants only the narrow rule surfaces used by the generated Instructions when they exist: `~/.aki/akidevrule`, `~/.claude/CLAUDE.md`, `~/.claude/CLAUDE.local.md`, and `~/.claude/skills/akirule`. It does **not** auto-grant the whole home directory, `~/.aki`, or `~/.claude`, so `~/.aki/mcpsv` OAuth state and `.claude` session/history files stay outside the fresh-install filesystem boundary.

Add/remove folders from **panel section 5**: click "+ Add folder…" and type an absolute path (`/Users/you/projects` or `C:\Users\you\projects`). Every row is owner-controlled and removable. Saving writes the authoritative folder list and takes effect immediately for every tool — shell, find, search, and file read/write/edit alike — no restart. To set an initial wider root without using the panel: `MCP_DATA_DIR=/other/path npm start` (or `set MCP_DATA_DIR=D:\work` then `npm start` on Windows cmd).
...
ChatGPT custom connectors use the MCP server URL directly and auto-discover OAuth/DCR metadata from `/.well-known/openid-configuration`. Developer mode must be enabled first; there is still no Advanced OAuth form or Registration URL to paste.

1. ChatGPT → Settings → Security and login → enable **Developer mode**.
2. ChatGPT → Settings → Connectors → New connector.
3. Pick an icon if wanted, then enter a name and description.
4. Under **Connection → Server URL**, paste the MCP URL (`https://your-machine.your-tailnet.ts.net/mcp`).
...
**Gemini Spark — custom MCP works, but approval is client-side per tool call:** Gemini can discover and invoke Aki tools, but Spark currently asks for approval before each MCP `tools/call`, including read-only calls. Aki cannot disable that client policy. For broad local codebase analysis, use `local__repo_snapshot` once with the project path; it returns a bounded tree plus prioritized source/config/docs without nested AI workers and avoids the 60-second `agent_read` deadline seen in Spark. Granular find/search/read remains fallback. Gemini's OAuth proxy callbacks remain allowlisted in `scripts/oauth.js`.
...
This integration is **Postman Desktop-first**. Aki does not require Postman Web, linked project folders, or durable team/workspace cloud state: use Desktop AI Chat, let Aki MCP (or an explicitly requested `dùng subagent shell` path) reach the Windows repo/files/shell, and keep long-task state in one shared Aki plan/taskKey plus `context_packet`/`task_checkpoint_*` instead of the chat transcript. The injected controller's **HANDOFF** action syncs the durable checkpoint and asks the current agent for one compact resume prompt; a fresh Desktop chat recovers that taskKey before continuing. When the chat UI cannot attach an image, drop the screenshot/photo into the local Postman image inbox and ask `xem ảnh mới nhất`: `local__image_inbox` returns MCP-native image content to the selected model without OCR or a separate Aki upload service; see [`docs/ref/postman-image-inbox.md`](docs/ref/postman-image-inbox.md). For iPhone access, remote-control the existing Windows desktop instead of moving the workflow to Postman Web/Cloud Agent; the preferred design is Cloudflare One private-network RDP + Access/MFA, documented in [`docs/ref/postman-desktop-remote.md`](docs/ref/postman-desktop-remote.md). Postman Desktop, Aki MCP, build/test, and repositories remain on Windows.

Windows can also opt into [`docs/ref/postman-pool-autojoin.md`](docs/ref/postman-pool-autojoin.md): the owner's Telegram user session (Telethon) listens to a group without adding a bot, accepts Postman invite links only from an allowlisted admin/chat ID pair, then Aki queues a Chrome/CDP pass over dedicated signed-in Chrome profiles and reports joined accounts through an outbound-only Telegram bot. Chrome uses a dedicated non-default user-data directory and loopback-only CDP; Aki dispatches normal pointer events only to the Postman `Accept Invite` / `Join Team` control. Human verification remains manual. The existing ROBOT SLTP webhook bot may be reused for `sendMessage`; Aki never calls `getUpdates`/`setWebhook`/`deleteWebhook` on it. Telegram credentials stay under `~/.aki/mcpsv/`, invite URLs are not logged, and temporary identity reads stay on D:.
...
`local__xkiro_read` uses xKiro's OpenAI-compatible tool-calling loop and gives the remote model only five scoped read primitives: repository snapshot, path search, content search, text-file read, and file metadata. Every path is hard-bounded to the requested `cwd`; write, edit, move, shell, Postman control, and other agent arms are not exposed inside the xKiro loop. `local__agent_read` delegates worker choice to the Budget Router: xKiro free first when healthy/configured, then AGY.

The integration is deliberately **free-only**: Aki reads xKiro's public live model catalog, exposes only `access_tier=free` models with tool calling in the panel selector, validates the saved choice server-side, and falls back only within that free tool-capable catalog if a selected model later disappears. Before each worker run it also checks the account's free-token remainder, bounds tool-loop steps and output tokens, defaults reasoning to `none`, and refuses to start a turn whose conservative estimate could exceed the remaining free allowance. `local__xkiro_status` (or the panel's **Check quota** / **Refresh models** actions) reports the current free catalog and token counter without exposing the key. No OpenAI SDK dependency is added; Node 22's built-in `fetch` talks to `https://api.xkiro.com/v1` directly.

## Aki Free-first Orchestrator

The fork keeps expensive lead models on judgment and routes retrieval through cheaper workers whenever the task allows it. The multi-step flow is: durable Project Graph + task checkpoint → `local__agent_read` automatic compression → Budget Router → compact lead packet → Astra/lead decision → native host delegation or scoped Aki write/edit tools → `local__run_cmd` verification → ledger/checkpoint update. One-off orientation still starts with `local__repo_snapshot`.

**Context Optimizer.** When enabled, `local__agent_read` automatically runs the free context-compression path and persists bounded optimizer/checkpoint activity. Automatic reads cap the packet at the lower of the panel budget and 4,000 tokens; an explicit `budgetTokens` may use up to 32,000, while `local__context_packet` uses the panel budget by default. Reuse the shared plan/task id as `taskKey` so follow-ups reuse the packet; call `local__context_packet` explicitly when a durable packet is needed before lead reasoning. Aki recovers the durable checkpoint, queries compact project knowledge, then asks the Budget Router for a free context-compression worker. The result is split into a stable prefix (`goal`, `constraints`, `decisions`, `architecture`, `acceptance`) and dynamic tail (`evidence`, `changes`, `tests`, `blockers`, `risks`), with prior compact items classified KEEP / STALE / WASTED. During the configured hot window (30 minutes by default) the stable prefix is preserved byte-for-byte only while it remains uncontradicted; if the worker marks an exact stable item STALE, Aki immediately forces a COLD rebuild. Compressor-reported `tests` never become checkpoint `lastGreen`; verification evidence must come from the execution/CI path. This is cache-aware shaping, not control of ChatGPT Work/Codex caching.

**Budget Router + Cost Ledger.** `local__budget_router_read` ranks observable eligible read workers as xKiro free → AGY quota, skipping unavailable/cooldown workers. xKiro's reported free remainder is authoritative when available. AGY quota exhaustion is learned only from real provider output (including reset duration when supplied) and persisted under `~/.aki/mcpsv/provider-status.json`; no percentage is invented. A local collector such as an Antigravity Language Server/Connect-RPC bridge may feed the same normalized `{providers:{...}}` shape through `AKI_PROVIDER_STATUS_FILE`, letting routing react to live remaining/reset data without coupling Aki to one collector implementation. `local__budget_router_status` reports AGY status and keeps provider-reported tokens, estimated input, estimated lead context avoided, and provider-reported cache hits as separate fields under `~/.aki/mcpsv/cost-ledger.json`.

**Project Graph.** `local__graph_sync` builds a compact derived graph only from durable project artifacts: `README.md`, `CHANGELOG.md`, package metadata, `docs/arch|feat|biz|ref`, and completed plans. It does not ingest source trees, chat transcripts, `.env`, credential/token/private-key files, or detected secret-like values; explicit task outcomes are secret-filtered before persistence. Entities carry source path/fingerprint, `lastVerified`, and confidence. `local__graph_query` combines exact matching, BM25-like lexical ranking, and trigram fuzzy matching so later tasks can recover decisions/outcomes without re-reading the whole repo. Sync/status also report file-cap, character-budget, and oversized-file coverage so a bounded graph cannot masquerade as complete.

**Checkpoint / recovery.** `local__task_checkpoint_save|get|list|recover` persists only task execution state: task key, cwd, plan path, active/completed/pending steps, blockers, latest green verification, and the compact stable/dynamic packet. It is not chat memory. Optimizer-enabled `agent_read` and explicit `context_packet` calls refresh an active checkpoint so compaction, account switching, or Aki restart can resume from a small recovery packet instead of replaying conversation history.

**Doctor.** `local__aki_doctor` is read-only. It reports loopback MCP/panel/OAuth readiness, roots/rules/trusted-directory conflicts, xKiro/AGY availability, and Context/Router/Graph/Checkpoint state. `deep=true` refreshes live xKiro status; Doctor never repairs configuration or widens permissions.

All free-first state is user-local under `~/.aki/mcpsv/`, atomically written and bounded. The panel's **Free-first** tab exposes Context budget/hot-window settings, cached task count, optimizer attempts/successes/failures/skips, last attempt/success/error, router/ledger statistics, Project Graph sync, checkpoint counts, and Doctor. The enabled mark is a configuration state, not proof of a recent successful optimizer run. Disabling Context Optimizer restores ordinary `agent_read` retrieval. Worker-reported provider tokens and Aki's estimated source/packet context are stored as separate metrics; estimated context savings are never presented as provider billing or cache hits.
...
local desktop MCP client
      │  http://127.0.0.1:19999/mcp (loopback-only Streamable HTTP, issued Bearer token required, no CORS)
      ▼
loopback-mcp.js ───────────────────────────────┐
      │                                        │
...
postman-pool-join.py — transient dedicated Chrome/CDP worker spawned only for an authorized pool invite
```
...
│   ├── tools-server.js           # one shared McpServer mounting shell/agy/xkiro/search/claude-mem/filesystem
...
Copy `.env.example` to `.env` and uncomment what you need — `start.js` loads it automatically on boot (falls back silently to defaults when `.env` is absent, so the default Tailscale flow is unaffected). Supported vars: `PUBLIC_ORIGIN` (`AKI_PUBLIC_ORIGIN` is accepted as a backward-compatible alias), `GATEKEEPER_PORT`, `PANEL_PORT`, `LOOPBACK_MCP_PORT`, `MCP_DATA_DIR`, `MCP_REQUEST_TIMEOUT_MS`, plus the optional shared Kimi/Qwen D1 mailbox set `AKI_D1_ACCOUNT_ID`, `AKI_D1_DATABASE_ID`, `AKI_D1_API_TOKEN`, `AKI_D1_POLL_MS`, `AKI_D1_LEASE_SECONDS` (default 900; stale `running` tasks are reclaimed after this lease). For a one-off alternate profile, pass `node --env-file=.env.user ./scripts/start.js` instead.
