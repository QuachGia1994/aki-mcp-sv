...
OpenCode is also a read-only fallback worker, but Aki does **not** duplicate its API key. Authenticate OpenCode Zen once with `opencode auth login`; OpenCode keeps that credential in its own auth store. The panel's **OpenCode** tab discovers live Zen models through the OpenCode CLI, shows only active zero-cost models with tool calling, stores only the selected model in `~/.aki/mcpsv/opencode.json`, and provides **Refresh models** plus **Test worker** actions. `local__opencode_read` uses the selected free model and, if it disappears from the current catalog, falls back only to another zero-cost Zen model; `local__opencode_status` reports auth, selected/effective model, fallback state, and the current free catalog without exposing credentials.
