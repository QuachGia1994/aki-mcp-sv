...
`local__xkiro_read` uses xKiro's OpenAI-compatible tool-calling loop and gives the remote model only five scoped read primitives: repository snapshot, path search, content search, text-file read, and file metadata. Every path is hard-bounded to the requested `cwd`; write, edit, move, shell, Postman control, and other agent arms are not exposed inside the xKiro loop. `local__agent_read` now delegates worker choice to the Budget Router: xKiro free first when healthy/configured, then OpenCode Zen zero-cost, then AGY/Kiro quota fallbacks.
...
OpenCode supplies two separate free-worker boundaries without duplicating its API key. Authenticate OpenCode Zen once with `opencode auth login`; OpenCode keeps that credential in its own auth store. The panel's **OpenCode** tab discovers the Zen catalog, shows only active zero-cost tool-calling models, stores the selected model plus a local **Enable write worker** toggle in `~/.aki/mcpsv/opencode.json`, and provides **Refresh models** plus **Test worker** actions. `local__opencode_read` stays strictly read-only. When the write toggle is enabled, `local__opencode_exec` uses the separate `Aki-exec` profile to edit only inside the requested project worktree; shell, web, delegation, skills, questions, todos, `.git`, `.opencode`, environment files, and external-directory edits remain denied. Aki injects both agent profiles through its dedicated `OPENCODE_CONFIG_DIR`, probes a bounded local port range so it never reuses an incompatible stale OpenCode server, and runs test/build commands separately through `local__run_cmd`. Both workers use only current zero-cost Zen models; if live catalog refresh fails, status/panel explicitly report `catalogSource=local-fallback` and the refresh warning instead of silently presenting the local CLI catalog as fresh.

## Aki Free-first Orchestrator

The fork keeps expensive lead models on judgment and routes retrieval/implementation through cheaper workers whenever the task allows it. The multi-step flow is: durable Project Graph + task checkpoint → `local__context_packet` → Budget Router → compact lead packet → Astra/lead decision → bounded `local__opencode_exec` → `local__run_cmd` verification → ledger/checkpoint update. One-off orientation still starts with `local__repo_snapshot`.

**Context Optimizer.** Reuse the shared plan/task id as `taskKey`. Aki recovers the durable checkpoint, queries compact project knowledge, then asks the Budget Router for a free context-compression worker. The result is split into a stable prefix (`goal`, `constraints`, `decisions`, `architecture`, `acceptance`) and dynamic tail (`evidence`, `changes`, `tests`, `blockers`, `risks`), with prior compact items classified KEEP / STALE / WASTED. During the configured hot window (30 minutes by default) the stable prefix is preserved byte-for-byte only while it remains uncontradicted; if the worker marks an exact stable item STALE, Aki immediately forces a COLD rebuild. Compressor-reported `tests` never become checkpoint `lastGreen`; verification evidence must come from the execution/CI path. This is cache-aware shaping, not control of ChatGPT Work/Codex caching.

**Budget Router + Cost Ledger.** `local__budget_router_read` ranks observable eligible read workers as xKiro free → OpenCode Zen zero-cost → AGY quota → Kiro quota, skipping unavailable/cooldown workers. xKiro's reported free remainder and OpenCode's live zero-cost catalog are authoritative where available. AGY/Kiro quota exhaustion is learned only from real provider output (including reset durations when supplied) and persisted under `~/.aki/mcpsv/provider-status.json`; no percentage is invented. A local collector such as an Antigravity Language Server/Connect-RPC bridge or Claude `rate_limits` reader may feed the same normalized `{providers:{...}}` shape through `AKI_PROVIDER_STATUS_FILE`, letting routing react to live remaining/reset data without coupling Aki to one collector implementation. `local__budget_router_status` reports those normalized provider statuses and keeps provider-reported tokens, estimated input, estimated lead context avoided, and provider-reported cache hits as separate fields under `~/.aki/mcpsv/cost-ledger.json`.
...
**Checkpoint / recovery.** `local__task_checkpoint_save|get|list|recover` persists only task execution state: task key, cwd, plan path, active/completed/pending steps, blockers, latest green verification, and the compact stable/dynamic packet. It is not chat memory. `context_packet` automatically refreshes an active checkpoint so compaction, account switching, or Aki restart can resume from a small recovery packet instead of replaying conversation history.

**Doctor.** `local__aki_doctor` is read-only. It reports loopback MCP/panel/OAuth readiness, roots/rules/trusted-directory conflicts, xKiro/OpenCode/AGY/Kiro availability, and Context/Router/Graph/Checkpoint state. `deep=true` refreshes live xKiro/OpenCode status; Doctor never repairs configuration or widens permissions.

All free-first state is user-local under `~/.aki/mcpsv/`, atomically written and bounded. The panel's **Free-first** tab exposes Context budget/hot-window settings, router/ledger statistics, Project Graph sync, checkpoint counts, and Doctor. Disabling Context Optimizer restores the previous snapshot/`agent_read` routing. Worker-reported provider tokens and Aki's estimated source/packet context are stored as separate metrics; estimated context savings are never presented as provider billing or cache hits.
