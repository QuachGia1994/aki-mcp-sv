...
OpenCode supplies two separate free-worker boundaries without duplicating its API key. Authenticate OpenCode Zen once with `opencode auth login`; OpenCode keeps that credential in its own auth store. The panel's **OpenCode** tab discovers the live Zen catalog, shows only active zero-cost tool-calling models, stores the selected model plus a local **Enable write worker** toggle in `~/.aki/mcpsv/opencode.json`, and provides **Refresh models** plus **Test worker** actions. `local__opencode_read` stays strictly read-only. When the write toggle is enabled, `local__opencode_exec` uses the separate `Aki-exec` profile to edit only inside the requested project worktree; shell, web, delegation, skills, questions, todos, `.git`, `.opencode`, environment files, and external-directory edits remain denied. Aki injects both agent profiles through its dedicated `OPENCODE_CONFIG_DIR`, probes a bounded local port range so it never reuses an incompatible stale OpenCode server, and runs test/build commands separately through `local__run_cmd`. Both workers use only the selected live zero-cost Zen model, with fallback only to another current zero-cost model; `local__opencode_status` reports auth, selected/effective model, write-worker state, fallback state, and free catalog without exposing credentials.
...
**Project Graph.** `local__graph_sync` builds a compact derived graph only from durable project artifacts: `README.md`, `CHANGELOG.md`, package metadata, `docs/arch|feat|biz|ref`, and completed plans. It does not ingest source trees, chat transcripts, `.env`, credential/token/private-key files, or detected secret-like values. Entities carry source path/fingerprint, `lastVerified`, and confidence. `local__graph_query` combines exact matching, BM25-like lexical ranking, and trigram fuzzy matching so later tasks can recover decisions/outcomes without re-reading the whole repo. Completed task checkpoints may append a compact outcome/decision record.
...
All free-first state is user-local under `~/.aki/mcpsv/`, atomically written and bounded. The panel's **Free-first** tab exposes Context budget/hot-window settings, router/ledger statistics, Project Graph sync, checkpoint counts, and Doctor. Disabling Context Optimizer restores the previous snapshot/`agent_read` routing. Metrics labeled "estimated" describe Aki context shaping only; they are never presented as provider billing or cache hits.
