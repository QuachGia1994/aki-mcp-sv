...
Windows can also opt into [`docs/ref/postman-pool-autojoin.md`](docs/ref/postman-pool-autojoin.md): the owner's Telegram user session (Telethon) listens to a group without adding a bot, accepts Postman invite links only from an allowlisted admin/chat ID pair, then Aki queues a local browser-profile pass and reports joined accounts through an outbound-only Telegram bot. LibreWolf/Selenium remains the default; an explicit `chrome-cdp` backend can instead launch a dedicated non-default Chrome user-data directory, attach Selenium through CDP, and dispatch normal pointer events to the Postman `Accept Invite` / `Join Team` control. Human verification remains manual in either backend. The existing ROBOT SLTP webhook bot may be reused for `sendMessage`; Aki never calls `getUpdates`/`setWebhook`/`deleteWebhook` on it. Telegram credentials stay under `~/.aki/mcpsv/`, invite URLs are not logged, and browser scratch stays on D:.
...
postman-pool-join.py — transient LibreWolf/Selenium or dedicated Chrome/CDP worker spawned only for an authorized pool invite
```
