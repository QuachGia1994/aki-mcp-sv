...
Windows also ships a no-install portable ZIP. Extract the ZIP before launching it, and keep `aki-watch.exe` beside the `aki-watch-runtime/` folder. `npm run portable:stage` copies the release executable and the exact `bundle.resources` map from `tauri.conf.json` into `src-tauri/target/release/portable/Aki-Watch-portable/`; CI first runs a hidden backend smoke that resolves and executes the bundled Node control script, then smoke-launches the GUI before publishing `aki-watch-windows-portable`. The portable app still requires the same host prerequisites listed above.
