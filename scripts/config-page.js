...
<div class="tabpane" id="tab-postman">
  <h3 class="subh">Control the Postman Desktop app</h3>
  <div class="updwarn"><strong>Desktop-only workflow:</strong> use Postman Desktop for AI Chat; do not switch to Postman Web/Cloud Agent or depend on linked folders/workspace cloud state. For iPhone remote use, remote-control this Windows session while Postman Desktop + Aki MCP + repo/shell/build/test stay on this PC. Preferred path: Cloudflare One private RDP + Access/MFA; never expose port 3389 directly. See <span class="mono">docs/ref/postman-desktop-remote.md</span>.</div>
...
<div class="tabpane" id="tab-context">
  ...
  <p class="helptext">The exposed <span class="mono">local__agent_read</span> path automatically runs the Context Optimizer when enabled; it prefers xKiro free quota then AGY before Astra/Work sees the compact packet. Use <span class="mono">taskKey</span> for follow-ups, or call <span class="mono">local__context_packet</span> explicitly when you need a durable packet. Provider-reported tokens, Aki estimates, avoided lead context, and cache hits stay separate metrics.</p>
  ...
  <p class="helptext">When enabled, broad <span class="mono">agent_read</span> calls automatically create or refresh a bounded packet; follow-ups with the same <span class="mono">taskKey</span> reuse it. Automatic reads cap the packet at the lower of this budget and 4,000 tokens; an explicit <span class="mono">budgetTokens</span> may use up to 32,000, while <span class="mono">context_packet</span> uses this budget by default. Contradicted stable facts trigger an immediate rebuild. The panel separates cached task count from attempts, successes, failures, skipped calls, and the latest task/error. This reduces Aki-to-lead context but does not claim or control ChatGPT/Work provider cache hits.</p>
