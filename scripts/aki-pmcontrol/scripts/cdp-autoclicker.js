...
(function () {
  ...
  const HANDOFF_PROMPT = 'Prepare a lean durable Aki handoff now. Sync the ONE shared plan, then call task_checkpoint_save with taskKey, cwd, planPath, active/completed/pending, blockers, and lastGreen; never store transcripts or secrets. Reply ONLY with a fresh-chat prompt that starts from current actionable state, reuses confirmed plan/checkpoint/git facts without re-research unless stale or ambiguous, and includes only taskKey/cwd/planPath, relevant branch/HEAD, intentional dirty files, current step, blockers, last-green verification, next concrete step, and any already-granted commit/push/deploy authorization. For substantive work target 30–100 lines, never exceed 100, and never pad just to reach 30. Omit narrative history and resolved detail. Instruct the fresh chat to call task_checkpoint_recover first, read the plan and current git state, call context_packet with the same taskKey, then continue the first unfinished checklist item.';
