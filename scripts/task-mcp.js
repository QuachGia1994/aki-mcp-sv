...
import { Shell, resolveExecFileTarget, validateAllowedCommandArgs } from './shell-mcp.js';
...
export function getProcessIdentity(pid) {
  ...
  try {
    if (process.platform === 'win32') {
      const script = `$p=Get-CimInstance Win32_Process -Filter "ProcessId = ${pid}" -ErrorAction SilentlyContinue; if($p){[pscustomobject]@{startedAt=$p.CreationDate.ToUniversalTime().ToString('o');commandLine=[string]$p.CommandLine}|ConvertTo-Json -Compress}`;
      const result = spawnSync('powershell', ['-NoProfile', '-Command', script], { encoding: 'utf8', windowsHide: true, timeout: 5000 });
      const text = (result.stdout || '').trim();
      return text ? JSON.parse(text) : null;
    }
    const started = spawnSync('ps', ['-p', String(pid), '-o', 'lstart='], { encoding: 'utf8', timeout: 5000 });
    if (started.status !== 0 || !(started.stdout || '').trim()) return null;
    const command = spawnSync('ps', ['-p', String(pid), '-o', 'command='], { encoding: 'utf8', timeout: 5000 });
    return { startedAt: started.stdout.trim(), commandLine: (command.stdout || '').trim() };
  } catch {
    return null;
  }
}

export function taskIdentityMatches(task) {
  if (!task?.processIdentity?.startedAt) return false;
  const current = getProcessIdentity(task.pid);
  return !!current && current.startedAt === task.processIdentity.startedAt;
}

function taskLooksAlive(task) {
  if (!task || task.status !== 'running') return false;
  if (task.processIdentity?.startedAt) return taskIdentityMatches(task);
  return isProcessAlive(task.pid);
}
...
export async function taskStart({ command, cwd, taskId: requestedId }) {
  ...
  const tasks = loadTasks();
  if (tasks[taskId] && taskLooksAlive(tasks[taskId])) {
    throw new Error(`Task "${taskId}" is already running (PID ${tasks[taskId].pid})`);
  }

  const shell = new Shell();
  const { bin, args } = shell.parse(command);
  const dir = resolveRealUnderRootSync(cwd);
  const permission = shell.checkPermission(bin, args);
  ...
  if (!pid) throw new Error('Failed to spawn background task process');
  ...
  for (let attempt = 0; attempt < 5 && !processIdentity; attempt += 1) {
    processIdentity = getProcessIdentity(pid);
    if (!processIdentity) await new Promise((resolve) => setTimeout(resolve, 40));
  }
  if (processIdentity) {
    const currentTasks = loadTasks();
    if (currentTasks[taskId] && currentTasks[taskId].pid === pid) {
      currentTasks[taskId].processIdentity = processIdentity;
      saveTasks(currentTasks);
    }
  }
...
export async function taskManage({ action, taskId, maxBytes = 10240, lines = null }) {
  ...
  if (action === 'list') {
    ...
    const list = Object.values(tasks).map((t) => {
      const alive = taskLooksAlive(t);
      if (!alive && t.status === 'running' && t.processIdentity?.startedAt) {
        t.status = 'exited';
        changed = true;
      }
      return {
  ...
  if (action === 'status') {
    const alive = taskLooksAlive(task);
    if (!alive && task.status === 'running' && task.processIdentity?.startedAt) {
      task.status = 'exited';
      saveTask(task);
    }
    let logSize = 0;
  ...
  if (action === 'stop') {
    const pidAlive = isProcessAlive(task.pid);
    if (pidAlive && !task.processIdentity?.startedAt) {
      throw new Error(`Refusing to stop task "${taskId}": process identity was not captured, so PID ownership cannot be verified.`);
    }
    const alive = taskLooksAlive(task);
    if (pidAlive && !alive) {
      throw new Error(`Refusing to stop task "${taskId}": PID ${task.pid} is alive but no longer matches the process that was started.`);
    }
    if (alive) killProcessGroup(task.pid);
    task.status = 'stopped';
    saveTask(task);
    return { taskId, pid: task.pid, stopped: true, wasAlive: alive };
  }
...
export function register(server) {
  server.registerTool(
    'task_start',
    {
      ...
      description: 'Start an allowlist-approved command in the background. Logs stream directly to disk. Returns taskId and pid for tracking with local__task_manage.',
      inputSchema: {
  ...
  server.registerTool(
    'task_manage',
    {
      ...
      description: 'Manage background tasks started with local__task_start: inspect status/liveness, tail bounded logs, stop only when process identity still matches, list tasks, or delete finished records.',
      inputSchema: {
