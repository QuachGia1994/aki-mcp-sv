...
import { register, isReadOnlySql } from '../scripts/sqlite-mcp.js';
...
async function testSqliteMcp() {
  ...
  try {
    ...
    // Test tool registration and strict read-only query classification
    ...
    assert.equal(isReadOnlySql('EXPLAIN QUERY PLAN SELECT * FROM users'), true);
    assert.equal(isReadOnlySql('PRAGMA table_info(users)'), true);
    assert.equal(isReadOnlySql('WITH x AS (SELECT 1) DELETE FROM users'), false);
    assert.equal(isReadOnlySql("ATTACH DATABASE 'outside.db' AS other"), false);
