...
### Changed
- **Local-First MCP engine — the Gatekeeper now binds `127.0.0.1:9999` unconditionally at startup** instead of pausing until a public ingress (Tailscale Funnel / Cloudflare Tunnel) is configured. Local tools (Cursor, Claude Code, AGY, Codex, Postman) connect directly with zero WAN round-trip and keep working fully offline; Bearer-token auth is still enforced on loopback. Ingress is now an optional satellite: `startGatekeeper()` accepts a `null` origin, the OAuth discovery/`/authorize` endpoints return `503` (not `404`) until an ingress is attached, the unauthenticated `/mcp` 401 emits a bare `Bearer` challenge (no bogus `null/…` metadata URL) when local-only, and `/register`+`/token` also gate to `503` without ingress (`scripts/gatekeeper.js`, `scripts/start.js`). Implements the engine portion of `task-1789505230187`.
- **Panel & docs for Local-First**: the panel's Postman connection snippet now targets `http://127.0.0.1:9999/mcp` (Postman Desktop is a local client), Section 0 is relabelled *Remote ingress (Web & Mobile AI) — optional* (no more "action required" when running local-only), and the README + `docs/ref/security-model.md` now document connecting Cursor / Claude Code / AGY / Postman directly over loopback plus the zero-trust loopback model. The panel also gains in-panel **Cursor / Claude Code / AGY / Codex** tabs with ready-to-paste loopback configs, and Section 1 is now split into two visually-labelled tab groups — *Local · direct 0ms* (Postman / Cursor / Claude Code / AGY / Codex, listed first, default tab Postman) and *Web · needs ingress* (Claude / Grok / ChatGPT / Gemini) — so a web-AI tab reads as needing ingress at a glance (`.tab-group-label`/`.tab-group-sep` in `public/panel.css`). The **Codex** tab and README block carry a `~/.codex/config.toml` snippet using streamable HTTP (`url` + `http_headers` with the bearer token inlined, no shell env var to export) (`scripts/config-page.js`, `public/panel.css`, `README.md`, `docs/ref/security-model.md`, `docs/index.md`). Completes `task-1789505230187` for all five local clients (agy / claude cli / codex / postman / cursor).
...
### Changed
- **Setup and connector onboarding now match the current v2 flow**: completed ingress setup collapses into a clear ready state, Claude setup only asks for a name and URL before OAuth confirmation, and the control panel better explains the governed v2 tool surface.
- **README now presents the full AKIMCP v2 capability set and current setup**: refreshed the product overview, removed stale Claude client-credential steps, and replaced the legacy screenshot with the v2 control-panel image.
