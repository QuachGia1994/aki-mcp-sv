...
- **Postman setup now follows Postman's official MCP Request → Generate Config → Agent Mode flow.** Manual Agent Mode JSON is documented as fallback only, and the panel warns about Postman's known stale HTTP-MCP reconnect state after a server restart.
- **Postman Agent Mode now connects with MCP 2026-07-28.** The public Streamable HTTP bridge accepts the new stateless `server/discover`, `tools/list`, `tools/call`, and `ping` flow, validates the required modern request headers, adds the required `resultType`/cache metadata, and keeps the existing 2025 sessionful path unchanged. CORS now allows `Mcp-Method`/`Mcp-Name` so browser-hosted modern clients can complete preflight.
- **Postman setup now makes the Bearer prefix impossible to miss.** Panel and README explicitly require `Authorization: Bearer <access-token>`, warn that a raw 64-character token returns 401, show the direct `url` + `headers` config shape, and tell users to discard any accidental `command`/`args` configuration.
...
### Fixed
- **Qwen Desktop no longer depends on the fragile editable `uvx aki-qwen-stdio` child.** Aki now exposes a loopback-only Streamable HTTP MCP endpoint on `127.0.0.1:19999` that reuses the same in-process tool/session policy as the public OAuth path. The local listener emits no CORS headers, rejects browser-origin requests and non-JSON POSTs, so Qwen can connect directly without OAuth, bearer plumbing, or an extra MCP child process.
