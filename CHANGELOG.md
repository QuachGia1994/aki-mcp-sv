...
- **`PUBLIC_ORIGIN` ingress escape hatch to bypass Tailscale** (`scripts/start.js`): set `PUBLIC_ORIGIN` to a stable public URL (e.g. a Cloudflare Tunnel subdomain that terminates TLS at its edge and forwards to the gatekeeper) and the server skips Tailscale entirely, using that origin as-is. Everything downstream keys off the single `origin` value, so no other code path changes. For regions where the Tailscale Funnel public edge is unreliable. Approach contributed via PR #4 (`@Ran-Xing`), TLS-termination path dropped since Cloudflare terminates at the edge.
