...
### Fixed
- `streamable-bridge.js`: session eviction at the `MAX_SESSIONS` cap now drops the **least-recently-used** session, not the oldest-created one. The Map's insertion order was being read as recency, so under load the first session created — usually the main long-lived conversation — was the first evicted. Reused sessions are now re-inserted to the tail so the eviction victim is genuinely the least active. Added `opened`/`reused` churn counters (logged only on a new-session event, no per-message spam) to diagnose whether the client resends `Mcp-Session-Id` — the open question tracked in `docs/plan/bridge-session-churn.md`.
