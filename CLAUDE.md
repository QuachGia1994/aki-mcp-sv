...
`scripts/streamable-bridge.js` keeps a bridge session alive as long as its upstream SSE to mcp-hub is open — no idle auto-close (a quiet session is not dead). Per-request timeout only (`MCP_REQUEST_TIMEOUT_MS`, default 10 min). Sessions die only when the hub restarts/closes the SSE, on client DELETE, or on the `MCP_MAX_SESSIONS` cap. Tokens survive restarts via `scripts/oauth.js` (persisted); the funnel does not, per RECURRING #1.
