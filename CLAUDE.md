...
- **Single shared session**: `scripts/streamable-bridge.js` maintains exactly **one** internal session for the process, held over an in-process `InMemoryTransport` pair (no child process, no SSE). OAuth-public `/mcp`, Bearer-authenticated loopback-only `/mcp`, and D1 bridge calls all reuse the same tool registry/session policy; external HTTP clients multiplex onto it via JSON-RPC ID remapping and `initialize` is answered locally from cache. Postman pool automation remains outside the MCP session topology: Aki Watch owns a separate local watcher process whose Telethon listener and transient Selenium/Python join workers never participate in the MCP session.
...
The core Aki server stays 1 Node process. `start.js` orchestrates: in-process gatekeeper (OAuth + public `/mcp`), Bearer-authenticated loopback-only Streamable HTTP MCP on `127.0.0.1:19999`, panel, optional `d1-bridge.js` polling, and a boot-time `warmToolsServer()` call. `npm run dev` isolates runtime data under `~/.aki/mcpsv-dev` and defaults gatekeeper/panel/loopback to `9997`/`9996`/`19998` so it can coexist with production. `scripts/tools-server.js` builds the one shared `McpServer`, mounting `shell`, structured read-only `git`/`sqlite`, background `task`, `port`, `agy`, optional `xkiro_read`/`xkiro_status`, `search`, optional read-only `claude-mem`, native `filesystem`, and read-only `postman_status`. Both HTTP entry points and the optional D1 bridge call the same `streamable-bridge.js` shared session, so local desktop clients and Qwen/Kimi do not add a second policy surface. Canonical tool names are `local__*`; the native filesystem tools also keep `filesystem__*` compatibility aliases for pre-1.10 bridge prompts. Folder scope (`scripts/roots.js`) is read fresh from `setting.json` on every call — a panel save takes effect on the next tool call, no restart. With no saved folders, the fallback is `process.cwd()` (or explicit `MCP_DATA_DIR`) plus only narrow rule surfaces under `~/.aki/akidevrule` and specific `~/.claude` rule files/router paths; whole home, `~/.aki`, and `~/.claude` are never fresh-install defaults. Claude-mem is an HTTP client module only: it may expose `local__claude_mem_search`, `local__claude_mem_timeline`, and `local__claude_mem_get_observations`, never capture/write/delete tools. xKiro is also network-only: `scripts/xkiro-mcp.js` uses built-in `fetch`, must remain free-tier/read-only by default, and gives the remote model no write/shell tool. The Postman control daemon under `scripts/aki-pmcontrol/` is an optional long-lived child process started only from the panel's Postman Launch action. Postman pool lifecycle is separate from `start.js`: `apps/aki-watch` calls `scripts/postman-pool-control.js` to start/stop one detached local watcher process, which owns the `postman-pool-telegram.py` Telethon child and transient `postman-pool-join.py` Selenium workers. `scripts/postman-pool.js` remains the authorization/dedupe/queue/report core used inside that watcher. None of these processes is an MCP server/session. There is no `mcp-hub` and no third-party filesystem child.
